...
export function mapRowToBusinessCard(
  ...
  return {
    ...
    avatarUrl: ((c.avatar_url ?? c.avatarUrl) as string) ?? null,
    coverUrl: ((c.cover_url ?? c.coverUrl) as string) ?? null,
...
export function mapRowToSummary(r: RawCardRow): BusinessCardSummary {
  return {
    ...
    cardKind: ((r.card_kind ?? r.cardKind) as CardKind) ?? "primary",
    status: ((r.status as CardStatus) ?? "draft"),
    publicMode: ((r.public_mode ?? r.publicMode) as PublicMode) ?? "members_only",
    displayName: ((r.display_name ?? r.displayName) as string) ?? null,
    professionalTitle: ((r.professional_title ?? r.professionalTitle) as string) ?? null,
    companyName: ((r.company_name ?? r.companyName) as string) ?? null,
    avatarUrl: ((r.avatar_url ?? r.avatarUrl) as string) ?? null,
