...
function PlatformPermissionsPage() {
  ...
  const handleRoleChange = (memberId: string, currentRole: string, currentDept: string, currentAssoc: string, newRole: string) => {
    setEdits(prev => ({
      ...
      [memberId]: {
        role: newRole,
        department: prev[memberId]?.department || currentDept || "Hội viên CEO 1983",
  ...
  const handleSave = async (member: any) => {
    ...
    const deptToSave = edit?.department || member.department || "Hội viên CEO 1983";
            ...
            <tbody>
              {loadingMembers ? (
                ...
                tc.pageRows.map((m: any, idx: number) => {
                  ...
                  const currentDept = edits[m.id]?.department ?? m.department ?? "Hội viên CEO 1983";
