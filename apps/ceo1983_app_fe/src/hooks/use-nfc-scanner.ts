 ...
 * Supports:
 * 1. Native Android WebView via CustomEvent ('ceo1983:nfc_tag', 'nfc_tag') & window message ('CEO1983_NFC_TAG', 'nfc_tag')
...
export function useNfcScanner(opts: { active: boolean; onDetect: (value: string) => void }) {
  ...
  useEffect(() => {
    ...
    const isCapacitor = Boolean(
      ...
      (window as unknown as { __CEO1983_LAST_NFC__?: any }).__CEO1983_LAST_NFC__ !== undefined
    );
    ...
    const onMessageEvent = (e: MessageEvent) => {
      if (e.data && (e.data.type === "CEO1983_NFC_TAG" || e.data.type === "nfc_tag")) {
        const payload = e.data.payload || e.data.detail || e.data;
        handleNativeNfcData(payload);
      }
    };

    window.addEventListener("ceo1983:nfc_tag", onCustomEvent);
    ...
    if (isCapacitor) {
      ...
      const lastNfc = (window as unknown as { __CEO1983_LAST_NFC__?: any }).__CEO1983_LAST_NFC__;
    ...
    return () => {
      stopped = true;
      window.removeEventListener("ceo1983:nfc_tag", onCustomEvent);
