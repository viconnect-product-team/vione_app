...
// 1. Sự kiện đã lưu / thêm vào lịch (ceo1983_saved_calendar_events)
...
export function ScheduleCalendar() {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("ceo1983:calendar-updated", handleUpdate);
  }, []);
  ...
  const eventItems: BcMobileTodayItem[] = useMemo(() => {
    ...
    for (const s of savedEvents) {
      ...
      list.push({
        ...
        category: "upcoming",
        urgency: "normal",
        counterpartDisplayName: s.organizer || "Sự kiện CEO 1983",
        action: {
    ...
    for (const e of crmEvents) {
      ...
      if (isReg) {
        ...
        list.push({
          ...
          category: "upcoming",
          urgency: "normal",
          counterpartDisplayName: e.associationName || (e as any).communityName || "Sự kiện CEO 1983",
          action: {
            labelKey: "bc.workHub.action.view",
    ...
    if (item.id.startsWith("event:")) {
      ...
      if (matched) {
        ...
        setSelectedEvent({
          ...
          location: savedMatched.location || "Tòa nhà CEO 1983",
