...
export function LandingHero({
  ...
  return (
    <section
      className={`relative pt-24 pb-20 md:pt-32 md:pb-28 overflow-hidden transition-colors duration-500 ${
        themeClass("bg-[#04060C]", "bg-[#FAF8F5]", "bg-black")
      }`}
    >
      {/* 1. High-Resolution Ultra Luxury Background Mesh & Ribbon Asset */}
      <div className="absolute inset-0 top-0 left-0 w-full h-[1200px] pointer-events-none z-0 overflow-hidden">
        <img
          src="/landing/business-hero-bg.jpg"
          alt="Business Connect Hero Background"
          className={`w-full h-full object-cover object-top transition-opacity duration-700 ${
            isDark
              ? "opacity-90 mix-blend-lighten"
              : isContrast
              ? "opacity-40"
          ...
          style={{
            WebkitMaskImage:
              "linear-gradient(to bottom, rgba(0,0,0,1) 0%, rgba(0,0,0,0.9) 70%, rgba(0,0,0,0) 100%)",
            maskImage:
              "linear-gradient(to bottom, rgba(0,0,0,1) 0%, rgba(0,0,0,0.9) 70%, rgba(0,0,0,0) 100%)",
          }}
        />
        {/* Subtle Stage Podium Overlay */}
        <img
          src="/landing/business-hero-stage.jpg"
          alt="Business Stage Background"
          className={`absolute inset-0 w-full h-full object-cover object-bottom transition-opacity duration-700 ${
            isDark
              ? "opacity-45 mix-blend-screen"
              : isContrast
              ? "opacity-20"
          ...
          style={{
            WebkitMaskImage:
              "radial-gradient(ellipse at 50% 80%, rgba(0,0,0,1) 25%, rgba(0,0,0,0) 80%)",
            maskImage:
              "radial-gradient(ellipse at 50% 80%, rgba(0,0,0,1) 25%, rgba(0,0,0,0) 80%)",
          }}
        />
      ...
      {/* 2. Expansive Ambient Gold Spotlight Halos & Light Rays */}
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
        <div
          className={`absolute -top-20 left-1/2 -translate-x-1/2 w-[1000px] lg:w-[1400px] h-[600px] lg:h-[800px] blur-[150px] transition-opacity duration-700 ${
            isDark
              ? "bg-gradient-to-b from-amber-400/35 via-yellow-600/20 to-transparent opacity-100"
              : isContrast
              ? "bg-white/10 opacity-50"
              : "bg-gradient-to-b from-amber-400/25 via-yellow-500/15 to-transparent opacity-80"
          }`}
        />
        <div
          className={`absolute top-1/4 right-4 w-[650px] h-[650px] rounded-full blur-[140px] transition-opacity duration-700 ${
            isDark ? "bg-amber-500/20 opacity-100" : isContrast ? "opacity-0" : "bg-amber-500/12 opacity-70"
          }`}
        />
        <div
          className={`absolute top-1/3 left-4 w-[550px] h-[550px] rounded-full blur-[150px] transition-opacity duration-700 ${
            isDark ? "bg-amber-600/15 opacity-100" : isContrast ? "opacity-0" : "bg-amber-600/8 opacity-50"
          }`}
        />
      </div>

      {/* 3. Architectural 3D Perspective Grid & Horizon */}
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
        {/* Top Radial Light Burst */}
        <div
          className={`absolute top-0 left-1/2 -translate-x-1/2 w-full h-[380px] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] ${
            isDark
        ...
        <svg className="absolute bottom-0 left-0 right-0 w-full h-[480px] opacity-[0.28]" xmlns="http://www.w3.org/2000/svg">
          <defs>
            ...
            <pattern id="hero-iso-pattern" width="40" height="40" patternUnits="userSpaceOnUse">
              <path d="M 40 0 L 0 0 0 40" fill="none" stroke="url(#hero-grid-fade)" strokeWidth="0.9" />
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#hero-iso-pattern)" />
        </svg>

        {/* Ambient Floating Gold Halo Rings */}
        <div
          className={`absolute top-20 right-[15%] w-[420px] h-[420px] rounded-full border animate-spin pointer-events-none ${
            isDark ? "border-amber-400/25" : isContrast ? "border-white/20" : "border-amber-600/15"
          }`}
          style={{ animationDuration: "35s" }}
        />
        <div
          className={`absolute top-32 right-[18%] w-[280px] h-[280px] rounded-full border border-dashed animate-spin pointer-events-none ${
            isDark ? "border-amber-300/30" : isContrast ? "border-white/20" : "border-amber-600/20"
          }`}
          style={{ animationDuration: "25s", animationDirection: "reverse" }}
        />

        {/* Floating golden star dust bokeh */}
        <div className="absolute top-[15%] left-[20%] w-2.5 h-2.5 rounded-full bg-amber-300 shadow-[0_0_15px_#FCD34D] animate-pulse" />
        <div className="absolute top-[32%] right-[25%] w-3 h-3 rounded-full bg-yellow-400 shadow-[0_0_20px_#FBBF24] animate-pulse" style={{ animationDuration: "3s" }} />
        <div className="absolute top-[48%] left-[10%] w-2 h-2 rounded-full bg-amber-200 shadow-[0_0_12px_#FDE68A] animate-pulse" style={{ animationDuration: "4s" }} />
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-center mb-16">
          ...
          <div className="lg:col-span-6 text-left space-y-6">
            {/* Eyebrow Badge */}
            <div
              className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full border text-[11px] sm:text-xs font-bold tracking-widest uppercase font-mono transition-colors duration-500 ${
                themeClass(
            ...
            {/* Display Headline */}
            <h1
              className={`text-4xl sm:text-5xl lg:text-[54px] xl:text-[58px] font-black tracking-tight leading-[1.14] uppercase transition-all duration-500 pb-1 ${
                themeClass(
            ...
            <p
              className={`text-base sm:text-lg max-w-xl leading-relaxed font-normal transition-colors duration-500 ${
                themeClass("text-slate-300", "text-[#475569]", "text-slate-200")
              }`}
            >
              {subtitle ||
            ...
            <div className="flex flex-wrap items-center gap-4 pt-2">
              {onCtaPrimaryClick ? (
                <button
                  ...
                  className={`shine-sweep inline-flex items-center justify-center gap-2 px-8 py-3.5 rounded-full font-extrabold text-sm sm:text-base transition-all transform hover:-translate-y-0.5 cursor-pointer shadow-xl ${
                    themeClass(
              ...
              {ctaSecondaryText &&
                (onCtaSecondaryClick ? (
                  <button
                    ...
                    className={`inline-flex items-center justify-center gap-2.5 px-6 py-3.5 rounded-full font-bold text-sm sm:text-base border transition-all cursor-pointer shadow-sm ${
                      themeClass(
                ...
                <div key={idx} className="text-left">
                  ...
                  <div
                    className={`text-xs font-medium mt-1 leading-tight ${
                      themeClass("text-slate-400", "text-[#64748B]", "text-slate-300")
                    }`}
                  >
                    {stat.label}
                  </div>
          ...
          <div className="lg:col-span-6 relative">
            ...
            <div
              className={`relative rounded-3xl p-3 border shadow-[0_30px_90px_rgba(0,0,0,0.85)] backdrop-blur-2xl transition-all duration-500 ${
                themeClass(
            ...
            >
              <div
                className={`rounded-2xl overflow-hidden border ${
                  themeClass("bg-[#070A12] border-white/[0.08]", "bg-slate-50 border-slate-200", "bg-black border-white/20")
                }`}
              >
                {/* Header Window Bar */}
                <div
                  className={`flex items-center justify-between px-4 py-3 border-b text-xs ${
                    themeClass("bg-[#0B0F1A] border-white/[0.08]", "bg-white border-slate-200", "bg-zinc-900 border-white/20")
                  }`}
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`ml-2 font-extrabold tracking-wide flex items-center gap-1.5 ${
                        themeClass("text-white", "text-slate-900", "text-white")
                      }`}
                    >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-[11px] font-semibold hidden sm:inline ${
                        themeClass("text-slate-300", "text-slate-700", "text-slate-200")
                      }`}
                    >
                      Nguyễn Văn Minh (VIP)
                    </span>
              ...
              <div className="hidden sm:block absolute -right-6 -bottom-8 w-56 lg:w-64 rounded-3xl p-2.5 bg-gradient-to-b from-[#F7D896]/40 via-[#0B0F1A] to-[#04060C] border-2 border-amber-300/60 shadow-[0_30px_70px_rgba(0,0,0,0.95)] backdrop-blur-3xl z-30 transform rotate-1 hover:rotate-0 transition-transform duration-300">
                <div
                  className={`rounded-2xl p-3.5 border text-left space-y-2.5 ${
                    themeClass("bg-[#070A12] border-white/[0.08]", "bg-white border-slate-200", "bg-black border-white/20")
                  }`}
                >
