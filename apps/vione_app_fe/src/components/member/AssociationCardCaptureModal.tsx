...
export function AssociationCardCaptureModal({
  ...
  return createPortal(
    ...
    >
      <div
        className="relative w-full max-w-lg bg-[#0C1322] text-white rounded-3xl shadow-2xl border border-amber-500/25 overflow-hidden flex flex-col my-auto max-h-[92dvh] animate-scale-in"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header - Executive Champagne Gold & Deep Obsidian */}
        <div className="p-4 sm:p-5 flex items-center justify-between border-b border-white/10 bg-[#070D18]">
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-2xl bg-gradient-to-br from-amber-500/20 to-amber-600/10 text-amber-400 border border-amber-500/30 shadow-xs">
              <Camera className="w-5 h-5" />
            </div>
            <div>
              <h3 className="font-bold text-sm sm:text-base text-white tracking-tight">Chụp & Tải Nền Danh Thiếp</h3>
              <p className="text-[11px] text-slate-400 font-medium">Khung ngắm tỷ lệ 1.7:1 chuẩn thẻ CEO 1983</p>
            </div>
          </div>
        ...
        {/* Tab Switcher */}
        {!capturedImage && (
          <div className="px-4 pt-3 pb-1 bg-[#0A101D]">
            <div className="flex items-center bg-black/40 p-1 rounded-2xl border border-white/5 text-xs font-semibold">
              <button
                type="button"
                onClick={() => {
                  setActiveTab("camera");
        ...
        {/* Footer Actions */}
        <div className="p-4 sm:p-5 border-t border-white/10 bg-[#070D18] flex flex-wrap items-center justify-between gap-3">
          {capturedImage ? (
            <>
              <button
                type="button"
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-4 py-2 rounded-xl text-xs font-semibold border border-white/20 text-white hover:bg-white/10 transition-colors flex items-center gap-1.5 cursor-pointer"
                >
                ...
                <button
                  type="button"
                  onClick={handleApply}
                  className="px-5 py-2.5 rounded-xl text-xs font-bold text-slate-950 bg-gradient-to-r from-amber-400 via-amber-500 to-amber-400 shadow-md shadow-amber-500/30 hover:brightness-110 active:scale-95 transition-all flex items-center gap-1.5 cursor-pointer"
                >
