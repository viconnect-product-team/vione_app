...
const LANGUAGES = [
  { code: "vi", label: "Tiếng Việt", codeLabel: "VI" },
  { code: "en", label: "English", codeLabel: "EN" },
  { code: "lo", label: "ພາສາລາວ", codeLabel: "LO" },
  { code: "km", label: "ភាសាខ្មែរ", codeLabel: "KM" },
  { code: "my", label: "မြန်မာဘာသာ", codeLabel: "MY" },
  { code: "ja", label: "日本語", codeLabel: "JA" },
  { code: "zh", label: "中文", codeLabel: "ZH" },
] as const;
...
export function LuxuryLangSwitcher({ className = "" }: { className?: string }) {
  ...
  return (
    <div ref={containerRef} className={`relative inline-block text-left font-sans ${className}`}>
      ...
      >
        ...
        <div className="relative z-10 flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-r from-[#AB6D3C] to-[#FDE6B4] p-[1px] shadow-[0_4px_12px_rgba(0,0,0,0.2)] dark:shadow-[0_4px_12px_rgba(0,0,0,0.5)] transition-all duration-300 group-hover:shadow-[0_0_12px_rgba(242,180,90,0.35)]">
          <div className="flex h-full w-full items-center justify-center rounded-full bg-white dark:bg-gradient-to-b dark:from-[#3a3937] dark:to-[#161514] text-slate-800 dark:text-white">
            <Globe className="h-3.5 w-3.5 text-amber-700 dark:text-[#f2b45a]" strokeWidth={1.8} />
          </div>
        </div>
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute right-0 mt-2.5 w-44 p-1.5 bg-white border border-amber-200/80 rounded-2xl shadow-[0_16px_40px_rgba(0,0,0,0.16)] backdrop-blur-xl z-[9999] animate-in fade-in slide-in-from-top-2 duration-150 dark:bg-[#0d1527] dark:border-[#AB6D3C]/30 dark:shadow-[0_20px_50px_rgba(0,0,0,0.7)]">
