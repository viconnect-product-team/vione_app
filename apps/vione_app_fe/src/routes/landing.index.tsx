...
function LandingPage() {
  ...
  return (
    ...
    >
      ...
      {/* Hero */}
      <section className="relative border-b border-border/30 px-6 pb-24 pt-20 md:pt-28 overflow-hidden">
        {/* Animated ambient background */}
        <div
          className="pointer-events-none absolute inset-0"
          style={{
            background:
              "radial-gradient(ellipse 80% 50% at 50% -10%, rgba(124,111,224,0.22) 0%, transparent 60%), radial-gradient(ellipse 40% 30% at 80% 50%, rgba(168,85,247,0.12) 0%, transparent 50%)",
          }}
        />
        {/* Subtle animated orb */}
        <div
          aria-hidden
          className="pointer-events-none absolute right-[10%] top-[-100px] h-[500px] w-[500px] rounded-full opacity-20 blur-3xl"
          style={{ background: "radial-gradient(circle, #7C6FE0, #a855f7, transparent)" }}
        />

        <div className="relative z-10 mx-auto max-w-6xl">
          {/* Painpoint badge */}
          <div className="inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/10 px-4 py-1.5 text-[12px] font-semibold text-primary mb-6">
            <span className="relative flex h-1.5 w-1.5">
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-primary/70" />
              <span className="relative inline-flex h-1.5 w-1.5 rounded-full bg-primary" />
            </span>
            {tx({ vi: "ViOne Enterprise Platform · ASEAN", en: "ViOne Enterprise Platform · ASEAN" })}
          </div>

          {/* Painpoint-first headline */}
          <h1
            className="mb-6 max-w-4xl text-[2.6rem] font-extrabold leading-[1.08] tracking-tight text-foreground md:text-[4rem] lg:text-[4.8rem]"
            style={{ fontFamily: "Sora, sans-serif" }}
          >
            {tx({ vi: "Mạng lưới quan hệ của bạn", en: "Your business network" })}{" "}
            <br className="hidden md:block" />
            <span
              style={{
                backgroundImage: "linear-gradient(135deg, #7C6FE0 0%, #a855f7 50%, #818cf8 100%)",
                WebkitBackgroundClip: "text",
                WebkitTextFillColor: "transparent",
                backgroundClip: "text",
              }}
            >
              {tx({ vi: "đang bị bỏ phí", en: "is leaving money" })}
            </span>{" "}
            <br className="hidden md:block" />
            {tx({ vi: "", en: "on the table" })}
          </h1>

          {/* Solution + hook */}
          <p className="mb-4 max-w-2xl text-[1.05rem] leading-relaxed text-muted-foreground md:text-[1.15rem]">
            {tx({
              vi: "80% giao dịch B2B thành công nhờ giới thiệu đúng người — nhưng hầu hết doanh nghiệp không biết ai đang ở cách họ 2 bước. ViOne thay đổi điều đó.",
              en: "80% of B2B deals close through the right introduction — yet most businesses don't know who's just 2 connections away. ViOne changes that.",
            })}
          </p>
          <p className="mb-10 max-w-xl text-sm leading-relaxed text-muted-foreground/70">
            {tx({
              vi: "Hợp nhất định danh · Danh thiếp thông minh · Mạng lưới quan hệ · Lịch họp · AI Trợ lý — trên một nền tảng.",
              en: "Identity · Smart Cards · Relationship Graph · Meeting Calendar · AI Assistant — all in one platform.",
            })}
          </p>
          ...
          <div className="flex flex-wrap gap-3 mb-16">
            <Link
              to="/auth"
              className="inline-flex items-center gap-2 rounded-xl px-7 py-3.5 text-sm font-bold text-white transition-all hover:scale-105 hover:opacity-90 shadow-lg"
              style={{ background: "linear-gradient(135deg, #7C6FE0, #a855f7)" }}
            >
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-primary/40 bg-primary/10 px-7 py-3.5 text-sm font-semibold text-primary transition-all hover:bg-primary/20"
            >
              {tx({ vi: "Đặt lịch demo", en: "Book a demo" })}
              <ArrowRight className="h-4 w-4" />
            </Link>
            <a
              href="#platform"
              className="inline-flex items-center gap-2 rounded-xl border border-border bg-card/50 px-7 py-3.5 text-sm font-semibold text-foreground transition-all hover:bg-secondary backdrop-blur-sm"
            >
          ...
          <div className="overflow-hidden rounded-2xl border border-border/60 bg-gradient-to-b from-card to-background shadow-2xl shadow-primary/10 backdrop-blur-xl">
                  ...
                  <div className="mt-4 flex items-end gap-1.5">
                    {s.bars.map((h, j) => (
                      <div
                        ...
                        style={{
                          height: `${h * 0.4 + 8}px`,
                          background: `linear-gradient(to top, rgba(124,111,224,0.25), rgba(168,85,247,${0.4 + j * 0.07}))`,
                        }}
                      />
                    ))}
                  </div>
