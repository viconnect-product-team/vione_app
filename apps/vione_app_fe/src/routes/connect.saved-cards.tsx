// BC-3.1 — Saved Business Cards Library.
//
// UI + client integration ONLY. Every read/mutation flows through the
// use-saved-cards hooks, which wrap SavedCardSDK. No table access, no server-fn
// imports, no networking logic here. Private notes/tags never enter the URL,
// the page <title>, telemetry or share surfaces.
...
export const Route = createFileRoute("/connect/saved-cards")({
  ssr: false,
...
function applySection(base: SavedCardsSearch, section: SavedCardsSection): SavedCardsSearch {
  switch (section) {
    case "favorites":
      return { ...base, section, favorite: true, archived: undefined, sort: base.sort };
    case "recent":
...
function SavedCardsPage() {
  ...
  // Normalize free-string URL values into the closed contract.
  ...
  return (
    <div className="mx-auto grid w-full max-w-6xl grid-cols-1 gap-6 px-4 py-6 md:grid-cols-[220px_1fr]">
      ...
      <aside className="space-y-4">
        <div>
          <h1 className="text-xl font-semibold text-foreground">{t("sc.title")}</h1>
          <p className="mt-1 text-sm text-muted-foreground">{t("sc.subtitle")}</p>
        </div>
        ...
        <nav aria-label={t("sc.section.all")} className="flex flex-row flex-wrap gap-1 md:flex-col">
          {SAVED_CARDS_SECTIONS.map((s) => (
            <button
              ...
              onClick={() => setSearch({ section: s, collection: "" })}
              className={
      ...
      <section aria-label={t("sc.title")}>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            ...
            <Input
              value={search.q}
              onChange={(e) => setSearch({ q: e.target.value.slice(0, 200) })}
          ...
          <Select value={search.sort} onValueChange={(v) => setSearch({ sort: normalizeSort(v) })}>
        ...
        {!isDefaultSearch(search) && (
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              size="sm"
              onClick={() =>
                navigate({ to: ".", search: () => ({ ...SAVED_CARDS_DEFAULT_SEARCH }) })
              }
            >
