...
## 4. BẢNG TIẾN ĐỘ BỔ SUNG CÁC TÍNH NĂNG MỚI ĐƯỢC NÂNG CẤP TOÀN DIỆN (7 CỘT QUY CHUẨN)

Bảng dưới đây thống kê toàn bộ các tính năng mới bổ sung và nâng cấp chuyên sâu cho App Hiệp Hội CLB Doanh Nhân CEO 1983 theo đúng 7 cột quy chuẩn nghiệp vụ bắt buộc:

| Chức năng chính | Chức năng nhỏ liên quan tới chức năng chính | Giao diện (Màn hình) | Mức độ ưu tiên | Mức độ hoàn thành | Mô tả | API tương tự / liên quan |
|---|---|---|:---:|:---:|---|---|
| **Chỉnh sửa hồ sơ nhanh tại Trang chủ** | Đổi ảnh đại diện (avatar), ảnh bìa (cover), tải logo công ty của hội viên lên ảnh bìa, cập nhật nhanh họ tên & số điện thoại trực tiếp | Trang chủ (`/association`, `QuickProfileEditModal`) | **Cao** | **100% (Done)** | Cho phép hội viên cập nhật nhận diện doanh nghiệp ngay lập tức như Facebook tại trang chủ, đồng bộ tức thì vào cache và phát sự kiện realtime | `PUT /api/members/me/profile`, `POST /api/media/upload`, `vba_member_company_logo_updated` |
| **Tinh gọn thanh tác vụ nhanh Trang chủ** | Bỏ nút Ưu đãi và Mã QR thừa; thêm nút Biểu quyết; loại bỏ hoàn toàn quét NFC và Apple Wallet | Trang chủ (`/association`) | **Cao** | **100% (Done)** | Tối ưu thanh tác vụ theo chuẩn hiệp hội doanh nghiệp, chuyển hướng trực tiếp vào các phiên biểu quyết cuộc họp và sự kiện thực tế | TanStack Router link `/association/voting` |
| **Thống kê Realtime Sàn Cơ hội Giao thương** | Thanh chỉ số thời gian thực: Tổng số cơ hội, Tổng số sản phẩm liên kết, Tổng giá trị giao dịch ước tính (VNĐ) | Cơ hội giao thương (`/association/opportunities`) | **Cao** | **100% (Done)** | Hiển thị bộ 3 chỉ số kinh tế realtime trên đầu trang cơ hội, tự động tính toán tổng giá trị deals từ các bài đăng cung cầu | `GET /api/opportunities/stats`, `listMyOpportunities` |
| **Khu vực Quảng cáo Tài trợ Hoàng Gia Marketplace** | Banner slider quảng cáo tài trợ doanh nghiệp thành viên với phong cách Obsidian Dark & Warm Amber Gold sang trọng; tự động trượt 4s | Sàn giao thương Marketplace (`/association/products`) | **Cao** | **100% (Done)** | Thay thế màu xanh đơn điệu cũ bằng thiết kế Titan Đen & Vàng Gold đẳng cấp dành riêng cho các doanh nghiệp tài trợ quảng bá sản phẩm | `GET /api/marketplace/sponsors` |
| **Nhắn tin trực tiếp Nhà bán hàng trên Sàn** | Tích hợp icon tin nhắn trực tiếp trên từng thẻ sản phẩm, mở ngay luồng chat 1-1 với người đăng sản phẩm để thương thảo | Sàn giao thương Marketplace (`/association/products` & `/association/messages`) | **Cao** | **100% (Done)** | Rút ngắn khoảng cách kết nối B2B, cho phép người mua trao đổi trực tiếp giá sỉ, chính sách chiết khấu và hợp đồng cung ứng | `POST /api/connect-app/dm/threads`, `sendMessage` |
| **Đăng bài Tin tức & Chia sẻ liên kết ngoài** | Cho phép hội viên đăng bài viết hoạt động, đính kèm link ngoài (Web/Facebook/Báo chí) với khung preview card trực quan | Bản tin CLB (`/association/news`) | **Cao** | **100% (Done)** | Giúp hội viên lan tỏa tin tức doanh nghiệp, có modal xem chi tiết và nút truy cập link gốc mượt mà | `POST /api/news`, `listNews` |
| **Thẻ Hội Viên VIP & Quản lý Quyền riêng tư** | Logo công ty hội viên ở góc trên bên phải; mã QR dưới logo bấm phóng to giữa màn hình; avatar hội viên; bỏ lật sau thẻ (No 3D flip); bỏ ảnh bìa lặp; hồ sơ & hợp đồng đặt dưới thẻ; modal căn giữa màn hình chuẩn màu sang trọng; Toggle bật/tắt hiển thị SĐT, Zalo, Email | Thẻ hội viên (`/association/card`, `Ceo1983BusinessCardVisit`, `EditCardModal`) | **Cao** | **100% (Done)** | Tái cấu trúc toàn diện thẻ hội viên sang trọng, trao quyền kiểm soát dữ liệu cá nhân cho từng hội viên khi đối tác quét mã | `PUT /api/members/me/card-privacy`, `GET /api/members/me` |
| **Tích hợp Quét mã QR trực tiếp dưới Thẻ hội viên** | Nút quét QR camera đặt ngay dưới thẻ hội viên, mở camera quét nhanh danh thiếp của đối tác tại sự kiện | Thẻ hội viên (`/association/card`) | **Cao** | **100% (Done)** | Tiện lợi quét mã giao lưu tại các buổi tiệc, gala, hội thảo mà không cần tìm kiếm tính năng ở nơi khác | `HTML5 QR Scanner API`, `/association/checkin` |
| **Tinh gọn Trang cá nhân (Tránh trùng lặp)** | Lược bỏ tường bài viết, danh sách bạn bè 600 dòng thừa; bỏ cài đặt riêng tư ở tab cá nhân; thay bằng thẻ định danh doanh nhân điều hướng sang Thẻ hội viên | Trang cá nhân (`/association/profile`) | **Cao** | **100% (Done)** | Tránh lặp lại tính năng giữa Trang cá nhân và Thẻ hội viên, giữ ứng dụng luôn tinh gọn và tốc độ load cao | TanStack Router navigation `/association/card` |
| **Danh thiếp số công khai quét ngoài ứng dụng** | Trang web xem thẻ công khai (`/card/:code`) có giao diện đồng bộ 1:1 với thẻ hội viên nội bộ, hiển thị Họ tên, SĐT, Công ty, Logo, Facebook, Zalo theo đúng cài đặt riêng tư | Danh thiếp số công khai (`/card/$code.tsx`) | **Cao** | **100% (Done)** | Khắc phục lỗi sai lệch giao diện trước đây, giúp đối tác bên ngoài quét mã là thấy ngay danh thiếp doanh nhân đẳng cấp | `GET /api/business-cards/code/:code` |
| **Hẹn gặp Kết nối & Bàn chiến lược kinh doanh B2B** | Bottom sheet trượt từ dưới lên nhập Tên, SĐT, Công ty, Mục đích hẹn gặp, đính kèm Cơ hội liên kết; gửi Thư mời vào hộp thư người nhận kèm nút "Đồng ý kết nối" và nút "Hủy" (có ô nhập lý do không bắt buộc) | Danh bạ hội viên & Hộp thư (`/association/members`, `/association/messages`, `BusinessConnectBottomSheet`) | **Cao** | **100% (Done)** | Định hình văn hóa kết nối đẳng cấp giữa các CEO: kết nối có mục đích hợp tác và lịch hẹn rõ ràng | `POST /api/connect-app/dm/messages`, `respondMemberConnectionFn` |
| **Tối giản nút bấm toàn ứng dụng (Icon-First UI)** | Chuyển đổi toàn bộ nút bấm chữ cồng kềnh trong danh bạ, danh thiếp và tin nhắn sang các icon Lucide thanh lịch; nút bấm chỉ giữ lại ở các hành động quan trọng (Chỉnh sửa hồ sơ, Đồng ý kết nối, Xác nhận biểu quyết) | Toàn bộ các màn hình App Hiệp Hội | **Cao** | **100% (Done)** | Giao diện thoáng đãng, sang trọng, giảm tải áp lực thị giác cho người dùng | `Lucide React Icons` |
| **Danh bạ liên hệ nhanh đầy đủ 7 Ban Chuyên Môn** | Hiển thị đầy đủ thông tin lãnh đạo, chức danh, hotline, email, Zalo của toàn bộ 7 Ban Chuyên Môn và tổng đài hỗ trợ 24/7; form gửi yêu cầu trực tuyến tới Ban Thư Ký | Modal Danh bạ & Hỗ trợ (`ContactSupportModal.tsx` tại `/association` & `/association/profile`) | **Cao** | **100% (Done)** | Minh bạch kênh liên lạc giữa hội viên và các ban điều hành chuyên trách của hiệp hội | `POST /api/support/tickets`, `tel:`, `mailto:` |
| **Hướng dẫn sử dụng 2 chế độ thông minh** | Chế độ 1 là tour chỉ dẫn từng bước (6 màn hình) có số bước, tiến trình % và nút "Bỏ qua" (Skip); Chế độ 2 là xem và tải sổ tay PDF chính thức; Nút upload file PDF được bảo vệ nghiêm ngặt chỉ hiển thị cho Admin / Ban Quản Trị | Modal Hướng dẫn sử dụng (`UserGuideModal.tsx` tại `/association/profile`) | **Cao** | **100% (Done)** | Giúp hội viên mới dễ dàng làm quen với app; ngăn chặn việc hội viên tự ý tải file rác lên hệ thống | `POST /api/media/upload`, `uploadFile` |
| **Biểu quyết trực tuyến đi kèm Sự kiện & Cuộc họp** | Gắn liền với từng sự kiện và cuộc họp thực tế; phân tách 2 tab "Đang diễn ra" và "Lịch sử biểu quyết"; kiểm phiếu thời gian thực với tỷ lệ % trực quan; ghi nhận kết quả nghị quyết đại hội | Màn hình Biểu quyết (`/association/voting`) | **Cao** | **100% (Done)** | Số hóa hoàn toàn quyền biểu quyết dân chủ của hội viên trong các kỳ đại hội và họp Ban Chấp Hành | `GET /api/events/my-events`, `POST /api/voting/ballot` |
| **Phân quyền Hội viên dành cho Admin & Ban Quản Trị** | Màn hình quản trị cho phép Admin tìm kiếm hội viên, phân bổ vào 7 Ban Chuyên Môn, bổ nhiệm chức vụ (Trưởng Ban, Phó Ban, Ủy Viên) và bật/tắt 6 quyền thao tác chi tiết trên App (Sự kiện, Tin tức, Marketplace, Biểu quyết, Quản lý Hội viên, Admin) | Màn hình Phân quyền (`/association/permissions` & `/association/profile`) | **Cao** | **100% (Done)** | Trao công cụ điều hành phân cấp cho Ban Thường Trực và Ban Quản Trị CLB CEO 1983 | `GET /api/members`, `PUT /api/permissions/member` |
| **Chuẩn hóa cấu trúc thư mục Common & Tối ưu hóa mã nguồn** | Đưa các thành phần dùng chung vào thư mục `components/common/` (`QuickProfileEditModal.tsx`, `BusinessConnectBottomSheet.tsx`), phân tách rành mạch logic nghiệp vụ và component hiển thị | Cấu trúc dự án `src/components/common` | **Cao** | **100% (Done)** | Mã nguồn sạch sẽ, dễ bảo trì, tái sử dụng tối đa, biên dịch 0 lỗi TypeScript | Frontend Modular Component Architecture |
| **Quy chuẩn màu sắc & Giảm thiểu lạm dụng chữ** | Áp dụng bảng màu Cobalt Navy (`#003B95`), Amber Gold (`#F59E0B`), Obsidian Slate (`#0B132B`); loại bỏ các khối chữ dày đặc, sử dụng huy hiệu và thẻ thông tin tinh gọn | Toàn bộ hệ thống giao diện | **Cao** | **100% (Done)** | Tạo ấn tượng thị giác chuyên nghiệp, đẳng cấp doanh nhân ngay từ cái nhìn đầu tiên | Executive Design Tokens / Tailwind CSS |
